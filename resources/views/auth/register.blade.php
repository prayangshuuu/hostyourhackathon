...
@slot('slot')
  <h1 class="text-xl font-bold text-slate-900 text-center">Create account</h1>
  <p class="text-xs text-slate-500 text-center mt-1.5 mb-7">Sign up to get started with the hackathon</p>

  <form method="POST" action="{{ route('register') }}" class="space-y-0">
    @csrf
    <x-input label="Full name" name="name" :value="old('name')" :error="$errors->first('name')" required autofocus />
    <x-input label="Email address" name="email" type="email" :value="old('email')" :error="$errors->first('email')" required />
    <x-input label="Password" name="password" type="password" :error="$errors->first('password')" required autocomplete="new-password" />
    <x-input label="Confirm password" name="password_confirmation" type="password" required autocomplete="new-password" />

    <div class="pt-2">
      <x-button type="submit" variant="primary" fullWidth size="lg">Create account</x-button>
    </div>
  </form>
